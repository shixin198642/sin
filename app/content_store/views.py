  ...
  if not docs:
    ...
    try:
      ...
      for doc in jsonDocs:
        ...
        if not valid:
          logging.warn("Found an invalid doc for store %s when adding docs. error: %s" % (store_name, error))
...
def updateDoc(request,store_name):
  try:
    ...
    if not doc:
      ...
      if not valid:
        logging.error("Found an invalid doc for store %s when updating a doc: %s" % (store_name, error))
        resp = {'ok': False,'numPosted':0, 'error':error}
        return HttpResponseBadRequest(json.dumps(resp))
    ...
    def _fix_url(files):
      for f in files:
        if not re.match(r'^https?://.*', f['url']):
          inner_ip = getattr(settings, 'LOCAL_INNER_IP', None)
          if inner_ip:
            f['url'] = 'http://%s:%s%s' % (inner_ip, settings.SIN_LISTEN, f['url'])
          elif request:
...
@login_required
def startStore(request, store_name, config_id=None, restart=False, node=None, with_running_info=True):
  return do_start_store(request, store_name, config_id, restart, node, with_running_info)
